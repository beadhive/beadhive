...
ordinary `*dispatch` calls are excluded.
...
- Compatibility surface: the six legacy modules, CLI/MCP call sites, lifecycle hook ordering,
  JSON/text/exit shapes, and 364 exact semantic patch/getattr/import seams found across all 387
...
- Compatibility surface: `plan`, `plan_repair`, `molecule`, `report`, `report_target`, `triage`,
  ...
  caller scope contains 50 exact semantic seams; current selected closure is ten legacy files /
...
- Compatibility surface: validation record schema/replay/identity behavior, state-stream cursor
  ...
  387-file caller scope contains 22 exact semantic seams; current selected closure is nine legacy
