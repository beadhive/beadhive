...
Beadhive's own `plugins/agf/skills/beadhive-concepts/` bundle and `docs/` in this repo.
...
- **The axis of divergence is not "how autonomous" — it is *who fixes the runtime and the autonomy
  ...
  - **Beadhive** makes the **runtime and autonomy level a dial the operator sets**. It is
...
**Punchline:** Beadhive is the **harness-agnostic control layer with an autonomy dial**; Gas\* are
**opinionated always-on runtimes**. Because they share the beads/Dolt substrate, Beadhive can *drive*
...
The upshot: a Beadhive rig's beads are *mechanically consumable* by a Gas\* runtime and vice-versa.
...
- **Hardcoded → config → harness-agnostic.** GasTown bakes roles into Go types and encodes
  ...
  "roles are examples, not platform law" — every role becomes user config (a **pack**). Beadhive's
...
- **Autonomy as a fixed choice vs a trust dial.** In Gas\* the daemon/orchestrator *is* the
  autonomy level. Beadhive treats autonomy as a **dial the operator turns as trust grows** —
...
- **Scheduler models differ** (detailed in §6): GasTown = capacity/back-pressure daemon; GasCity =
  no central scheduler (ordering emerges from dependency edges); Beadhive = a **planning-time**
...
| **Dispatch** | `gt sling` + capacity-controlled **scheduler daemon** | `gc sling` → compiled **control-bead graph** (check/retry/fan-out/tally/drain) | `bh work assign`/`claim` with **fanout / collapsed / auto** modes |
| **Scheduling** | back-pressure daemon (`scheduler.max_polecats`, batch size/heartbeat, **circuit breaker** at 3 failures) | **no central scheduler** — ordering emerges from blocking `needs` edges | **planning-time** `bh work schedule`: forms groups (child epics / planner batches / private chains / singletons) under **four guards** (cohesion, size cap, single model tier, single review gate) |
...
dependencies), and Beadhive schedules at **plan time** for **cohesion and blast-radius** (a batch
...
expose no MCP server; Beadhive is MCP-native with typed tools + subscription resources.** That is the
...
- **MCP-native structured control has no Gas\* equivalent** — typed config/bead/plan/label tools +
  always-fresh subscription resources, versus Go-CLI + tmux keystroke injection. This is Beadhive's
...
2. **Harness-agnostic runtime + autonomy dial.** Beadhive abstracts the runtime away and scales from
...
- **Data-layer interop is the anchor.** All three read/write beads on `refs/dolt/data`; a Beadhive
  rig *is* a consumable GasCity rig (isolation by bead-ID prefix / triplet labels). No translation
  layer needed.
- **Beadhive plans + gates; a Gas\* runtime can execute the fleet.** Beadhive's planning plane
  (planner + analyst, two gates) produces exactly the gated molecule/graph GasCity's v2 orchestrator
  wants to fan out. Being harness-agnostic, Beadhive can *drive* that runtime as its autonomy dial
  turns up — the Gas\* daemon becomes **one harness Beadhive targets, not a competitor**.
- **Beadhive's governance is what Gas\* underspecify** — explicit plane separation (roles *and*
  lifecycle), entity-agnostic seats, a first-class reviewer, the opinionated git-history model, the
  Contribution/external-quality plane, and **control-plane** commissioning. Position Beadhive as the
  **graduated-trust control layer** over whichever runtime executes.
- **Beadhive's seat model could ship *as a GasCity pack*.** GasCity explicitly says "roles are
