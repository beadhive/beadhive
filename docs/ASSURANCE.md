...
(Implementation: `src/beadhive/guard.py` — `is_security_gate` — classifies the gate; the
warden-only resolution policy lives with `bh work approve` in
`packages/beadhive-core/src/beadhive_core/review.py`.)
