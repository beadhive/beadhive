...
def _role_headless(
    ...
    *,
    ...
    typer.echo(f"→ {seat}: headless via {plan.backend} — {plan.detail}", err=True)
    ...
    if qualified is not None:
        if entry is None:
            ...
            raise typer.Exit(1)
        journal = role_execution.create_role_journal(
            qualified, hive=registry.hive_key(entry), bead=bead
        )
        if journal.run_id == provider_continuation:
            # The values come from independent UUID mints, but keep the contract constructive:
            # even an injected deterministic UUID source cannot collapse the two identities.
            provider_continuation = f"provider-{provider_continuation}"
            session_index = argv.index("--session_id") + 1
            argv[session_index] = provider_continuation
        context = json.dumps(
            {
                ...
                "manifest_digest": qualified.manifest_digest,
            },
            sort_keys=True,
            separators=(",", ":"),
        )
...
def role_cmd(
    ...
    baml_required: bool = typer.Option(
        False,
        "--baml-required",
        help="require an exact provider-qualified BAML artifact; never fall back to direct Hitch.",
    ),
    ...
    if not name:
        launch_only_flags = [
            ...
            for flag, present in (
                ("--bead", bool(bead)),
                ("--hive", bool(hive)),
                ("--available-seat", bool(available_seat)),
                ("--current-seat", bool(current_seat)),
                ("--model", bool(model)),
                ("--effort", bool(effort)),
            )
            if present
        ]
        if launch_only_flags:
            rendered = ", ".join(launch_only_flags)
            typer.echo(f"✗ a seat is required when using launch options: {rendered}", err=True)
            raise typer.Exit(1)
    ...
    if baml_required:
        typer.echo(
            "✗ --baml-required applies to a headless --task/-d launch (or --explain)", err=True
        )
        raise typer.Exit(1)

    # A bare ``bh role`` is the read-only seat listing. Keep it entirely outside the workspace
    # path, even though ``_apply_role_workspace("", "")`` is currently a no-op: calling a
    # mutation-owning seam at all makes the listing's safety depend on that implementation detail.
    if not name:
        hitch_plugin.route(
            "",
...
@wt_app.command(
    "prune", help="remove SAFE managed worktrees (or one hive's) + prune admin files."
)
