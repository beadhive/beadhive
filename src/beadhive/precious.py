...
def _run_git(*args, **kwargs):
    """Run the scanner's read-only git query without importing the worktree facade.

    The scanner remains patchable at this exact symbol, while the dedicated seam prevents the
    safety policy from pointing back into its worktree consumer.  Scrubbing ambient ``GIT_*``
    matches the established worktree runner so an enclosing hook cannot override ``git -C``.
    """
    command = args[0]
    ...
    environment = {
        key: value
        for key, value in (os.environ if supplied_env is None else supplied_env).items()
