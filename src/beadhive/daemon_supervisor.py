...
from .kernel.daemon.contracts.supervision import (
    PLATFORM_LIFECYCLE_HANDOFF as PLATFORM_LIFECYCLE_HANDOFF,
)
from .kernel.daemon.contracts.supervision import (
...
if TYPE_CHECKING:
    from .daemon_local_state import DaemonKey, DaemonStatus
...
def run_lifecycle(
    ...
    *,
    ...
    from .daemon_local_state import current_daemon_key

    expected = key or current_daemon_key()
...
class DaemonServiceStatus:
    ...
    def payload(self) -> dict[str, Any]:
        from .daemon_local_state import DaemonPaths
...
def daemon_service_status(
    *,
    ...
    from . import daemon_local_state
    ...
    expected = key or daemon_local_state.current_daemon_key()
    manager = backend or get_supervisor_backend()
    supervisor = _require_identity(expected, manager.status(expected))
    local = daemon_local_state.daemon_status(expected)
    record = local.record
    listener = ListenerStatus(
