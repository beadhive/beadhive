...
**never ad-hoc labels**. The module `ws/state.py` is the single owner of the closed
...
These are registered as **closed dimensions** (`ws/state.py:STATE_DIMENSIONS`, merged in by
`registry.closed_dimensions`), so intake/outbound/origin beads validate clean under `ws labels
...
2. **Intake channel** = the closed `origin` dimension above (set via `bd set-state origin=…`,
   like `intake`). `ws report` files via **plain `bd create` + `bd set-state origin=report`** —
...
3. **Reporter identity** = `bd --actor` (unchanged) — **never** a closed label (`reported-by` is
   open-ended and would fail `ws labels validate`). Do not add a reporter label dimension.

### Fielding intake (triage duty)

Incoming reports must be **fielded, not buried in backlog**. The queue is **source-agnostic**:
`ws report` (`origin:report`), GitHub-issue import (`github`) and legacy import (`import`) all land
...
- **See the queue:** `ws work intake` — this rig's untriaged intake (source-agnostic; the resolved
...
- **Future follow-up:** cross-rig `ws hq` interchange (`ws plan` / `ws work --rig <id>`) is not
  wired yet.
