...
if TYPE_CHECKING:
    ...
    from ..models.issue_metadata import IssueMetadata
...
@_attrs_define
class Issue:
    """A tracked work item. Property semantics documented here apply to every schema that repeats them below.

    Attributes:
        id (str): Canonical issue id, e.g. `bd-abc`.
        title (str):
        priority (int): 0 (most urgent) through 4. Always present; 0 is a real value.
        ...
        notes (str | Unset):
        spec_id (str | Unset):
        status (str | Unset): `open`, `in_progress`, `blocked`, `closed`, or any custom status configured for the
            workspace. NOT a closed vocabulary: clients must treat an unrecognized value as "some status I do not know".
        issue_type (str | Unset): `bug`, `feature`, `task`, `epic`, `chore`, `gate`, and workspace configured types. Not
            a closed vocabulary.
        is_blocked (bool | Unset): The persisted readiness projection — true when an open blocking dependency keeps this
            issue out of the ready set. Derived from the dependency graph; never set by a client.

            NOT POPULATED by the /v0 read endpoints today: they hydrate issues without it, so the field is absent from their
            responses and its absence says nothing about whether an issue is blocked. Use `/v0/beads/ready` or the
            dependency endpoints to answer that. The field is carried by events-journal snapshots, which is where it is read
            back, and it is documented here because those snapshots serialize the same type this schema is pinned to.
        assignee (str | Unset): The actor currently holding the issue, if any.
        owner (str | Unset):
        ...
        lease_expires_at (datetime.datetime | Unset): Expiry of the current claim's lease, when one is active.
        heartbeat_at (datetime.datetime | Unset):
        lease_granted_node (str | Unset): The replica that granted the lease. Empty means the provenance is unknown,
        ...
        metadata (IssueMetadata | Unset): Arbitrary caller-supplied JSON object. VALUES MAY BE OF ANY JSON TYPE —
            string, number, boolean, array or nested object — because typed values enter through the explicit JSON metadata
            path and persist in older rows. Clients MUST NOT decode this into a string-to-string map; a strict decode fails
            on the first typed value and takes the whole response with it. That prohibition is on assuming a narrower type
            when reading THIS document: a profile MAY declare a narrower value schema under the rules in **Profiles**, and a
            client of that profile may rely on the profile's declaration.

            The object-at-top-level shape is the contract every producer and every metadata filter (`metadata_field`,
            `has_metadata_key`) assumes, but be aware the store validates only that the value is WELL-FORMED JSON: a row
            created with a non-object (e.g. `bd create --metadata '[1,2]'`) can carry an array or a scalar here. That is a
            storage-side gap, not licence for a producer to emit one — but a tolerant client should skip such a row rather
        ...
        dependencies (list[Dependency] | Unset): Dependency edges. Populated in export/interchange contexts; the detail
            endpoint returns richer entries — see `IssueDetails`.
        comments (list[Comment] | Unset):
        sender (str | Unset):
        ephemeral (bool | Unset): Ephemeral records are not synced.
        no_history (bool | Unset):
        wisp_type (str | Unset):
        storage_class (str | Unset): The record's history/replication contract. Empty means unset, which resolves to
            ephemeral for wisp-plane records and versioned otherwise.
        pinned (bool | Unset):
        is_template (bool | Unset):
        bonded_from (list[BondRef] | Unset):
        await_type (str | Unset): Gate condition type.
        await_id (str | Unset):
        timeout (int | Unset): NANOSECONDS. This is a Go `time.Duration`, which encodes as an int64 nanosecond count — a
            pre-existing wire fact, not a choice.
        ...
        actor (str | Unset): For event records, the entity that caused the event. Unrelated to the claiming actor in
            `ClaimRequest`.
        target (str | Unset):
        payload (str | Unset):
    """
    ...
    metadata: IssueMetadata | Unset = UNSET
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        from ..models.issue_metadata import IssueMetadata  # noqa: PLC0415
        ...
        metadata: IssueMetadata | Unset
        if isinstance(_metadata, Unset):
            metadata = UNSET
        else:
            metadata = IssueMetadata.from_dict(_metadata)
