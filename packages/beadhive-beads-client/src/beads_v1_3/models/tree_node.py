...
if TYPE_CHECKING:
    ...
    from ..models.tree_node_metadata import TreeNodeMetadata
...
@_attrs_define
class TreeNode:
    """One node of a walked dependency tree: a full issue plus where the walk reached it. Property semantics for the issue
    members are documented on `Issue`.

    The tree is FLAT. A node's place in it is read from `depth` and `parent_id`, not from nesting, and a subtree is
    contiguous in `items`.
        ...
        Attributes:
            ...
            depth (int): How far the walk was from the root when it reached this node. The root is 0.

                For a `both` walk each half measures its own depth from the root, so a depth does not say which half a node came
                from and nothing else on the node does either.
            parent_id (str): The node this one was reached FROM, empty on the root. A node is reached ONCE per walk, by the
                first path that got there, so a shared child of two parents names only the first of them here and the second has
                no visible edge to it.
            truncated (bool): ALWAYS FALSE. No implementation sets it: a node beyond `max_depth` is ABSENT from the answer
                rather than present and flagged, so a client cannot tell a tree that ended from one that was cut. It is
            ...
            metadata (TreeNodeMetadata | Unset):
            ...
            target (str | Unset):
            payload (str | Unset):
            edge_from_parent (str | Unset): The dependency type of the edge that led here, e.g. `blocks`, `parent-child`,
                `discovered-from`. Absent on the root. Not a closed vocabulary.
    """
    ...
    metadata: TreeNodeMetadata | Unset = UNSET
    ...
    edge_from_parent: str | Unset = UNSET
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        if edge_from_parent is not UNSET:
            field_dict["edge_from_parent"] = edge_from_parent
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        from ..models.tree_node_metadata import TreeNodeMetadata  # noqa: PLC0415
        ...
        if isinstance(_metadata, Unset):
            ...
            metadata = TreeNodeMetadata.from_dict(_metadata)
        ...
        edge_from_parent = d.pop("edge_from_parent", UNSET)
        ...
        tree_node = cls(
            ...
            target=target,
            payload=payload,
            edge_from_parent=edge_from_parent,
        )
