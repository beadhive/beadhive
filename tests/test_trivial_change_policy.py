...
Fixture paths deliberately use ``guide/``, never ``docs/``. The directory name is arbitrary to
every assertion here, and ``scripts/check_pants_proven.py`` classifies any test containing a
``docs/``-prefixed string literal as a runtime documentation READER that needs a manifest
entry. This test touches no filesystem at all; declaring it a doc reader would record a
dependency it does not have. Do not "fix" these fixtures back to ``docs/``.
...
KEYS = tuple(
    ...
    for n in ("guide", "unit", "integration", "stateful")
)


def attest(*, enabled=True, paths=("guide/*.md",), keys=("guide",)):
    return AttestConfig(
        keys=list(KEYS),
...
def test_all_paths_matching_is_yes():
    assert path_verdict(attest(), ("guide/design/a.md", "guide/b.md")) == TRIVIAL_YES


def test_one_unmatched_path_is_no():
    """A single code file restores the normal route: the policy is all-or-nothing."""
    assert path_verdict(attest(), ("guide/a.md", "src/beadhive/work.py")) == TRIVIAL_NO
...
def test_unconfigured_is_unknown_not_no():
    ...
    assert path_verdict(attest(enabled=False), ("guide/a.md",)) == TRIVIAL_UNKNOWN
    assert path_verdict(attest(paths=()), ("guide/a.md",)) == TRIVIAL_UNKNOWN
    assert path_verdict(attest(keys=()), ("guide/a.md",)) == TRIVIAL_UNKNOWN
...
def test_trivial_change_runs_only_the_configured_keys():
    selected, record = trivial_selection(attest(), ("guide/a.md",), KEYS)
    assert [k.name for k in selected] == ["guide"]
    assert record["applied"] is True and record["ran"] == ["guide"]
...
def test_policy_never_skips_every_key(enabled):
    ...
    selected, record = trivial_selection(
        attest(enabled=enabled, keys=("nonexistent",)), ("guide/a.md",), KEYS
    )
...
def test_triage_yes_cannot_skip_on_its_own():
    ...
    selected, record = trivial_selection(
        attest(paths=("nope/*",)), ("guide/a.md",), KEYS, triage=lambda _p: TRIVIAL_YES
    )
    assert selected is None
...
def test_agreement_applies_the_fast_path():
    selected, record = trivial_selection(
        attest(), ("guide/a.md",), KEYS, triage=lambda _p: TRIVIAL_YES
    )
    assert [k.name for k in selected] == ["guide"]
...
def test_triage_unknown_still_allows_the_glob_fast_path():
    ...
    selected, _ = trivial_selection(
        attest(), ("guide/a.md",), KEYS, triage=lambda _p: TRIVIAL_UNKNOWN
    )
    assert [k.name for k in selected] == ["guide"]
...
def test_triage_that_raises_is_unknown_and_never_fails_the_run():
    ...
    selected, record = trivial_selection(attest(), ("guide/a.md",), KEYS, triage=boom)
    assert [k.name for k in selected] == ["guide"]
    assert "network down" in record["triage_error"]


def test_triage_returning_garbage_is_treated_as_unknown():
    selected, record = trivial_selection(attest(), ("guide/a.md",), KEYS, triage=lambda _p: "maybe")
    assert [k.name for k in selected] == ["guide"]
...
def test_absent_trivial_section_defaults_to_off():
    ...
    assert path_verdict(cfg, ("guide/a.md",)) == TRIVIAL_UNKNOWN
...
def test_fnmatch_star_crosses_directory_separators():
    ...
    These are fnmatch patterns, not pathlib globs: '*' crosses '/', so 'guide/*' silently
    admits 'guide/schemas/thing.json' -- a checked-in schema that code may well load. The
    semantic triage's veto is what makes that survivable.
    """
    assert path_verdict(attest(), ("guide/deeply/nested/a.md",)) == TRIVIAL_YES
    assert path_verdict(attest(paths=("guide/*",)), ("guide/schemas/thing.json",)) == TRIVIAL_YES
    assert path_verdict(attest(paths=("guide/*",)), ("src/beadhive/work.py",)) == TRIVIAL_NO
