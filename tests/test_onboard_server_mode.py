...
def test_existing_hive_skip_path_never_calls_server_mode_wiring(tmp_path, monkeypatch):
    """The idempotent skip (a VERIFIABLY WORKING store — `hive.store_opens`, bh-areg.7's
    review round 4, not merely `.beads/` existing) must return before `bd init`, before
...
# ---------------------------------------------------------------------------
# THREE branches, not two (bh-areg.7's review, round 4): `store_opens` is the real open
# test, and `reclaim_or_refuse` is the single authority for what a non-opening `.beads/`
# means. See test_onboard_server_mode_int.py::test_interrupted_mid_init_never_reported_ready
# for the real-bd SIGKILL reproduction this whole branch exists for.
# ---------------------------------------------------------------------------


def test_store_opens_false_when_beads_dir_absent(tmp_path):
    assert hive.store_opens(tmp_path) is False


def test_store_opens_true_when_the_probe_succeeds(tmp_path, monkeypatch):
    (tmp_path / ".beads").mkdir()
    monkeypatch.setattr(hive, "run", lambda cmd, **kw: _Result(returncode=0))
    assert hive.store_opens(tmp_path) is True


def test_store_opens_false_when_the_probe_fails(tmp_path, monkeypatch):
    """The reviewer's exact finding: `.beads/` present — even with a persisted `dolt_mode` —
    is not "working" on its own. Only a real `bd status` probe settles it."""
    import json

    (tmp_path / ".beads").mkdir()
    (tmp_path / ".beads" / "metadata.json").write_text(json.dumps({"dolt_mode": "server"}))
    monkeypatch.setattr(hive, "run", lambda cmd, **kw: _Result(returncode=1))
    assert hive.store_opens(tmp_path) is False


def test_store_opens_fails_closed_on_a_wedged_or_missing_probe(tmp_path, monkeypatch):
    """Never lets an inconclusive probe read as "yes": a timeout or a missing `bd` binary is
    treated the same as a probe that returned failure."""
    ...
    monkeypatch.setattr(hive, "run", _wedged)
    assert hive.store_opens(tmp_path) is False

    def _missing(cmd, **kw):
        raise OSError("bd: command not found")

    monkeypatch.setattr(hive, "run", _missing)
    assert hive.store_opens(tmp_path) is False


def test_reclaim_or_refuse_reclaims_wreckage_with_no_persisted_dolt_mode(tmp_path):
    (tmp_path / ".beads").mkdir()
    (tmp_path / ".beads" / "junk").write_text("partial init leftovers")

    hive.reclaim_or_refuse(tmp_path)  # must not raise

    assert not (tmp_path / ".beads").exists()


def test_reclaim_or_refuse_refuses_a_persisted_dolt_mode_via_unusable_store_never_raw(tmp_path):
    """The reviewer's SIGKILL-at-4.5s window: `dolt_mode` was written before the store was
    usable — but this could ALSO be a real, complete store whose server merely isn't
    reachable right now, so `reclaim_or_refuse` never guesses. It raises the dedicated
    `UnusableStore` — never the bare `RuntimeError` `cleanup_failed_bd_init` itself raises,
    which is exactly what reached `cli.py`'s generic handler as a structlog blob in round 4's
    own bug report."""
    import json
    ...
    with pytest.raises(hive.UnusableStore):
        hive.reclaim_or_refuse(tmp_path)

    assert (tmp_path / ".beads").exists()  # refused, never guessed away


# ---------------------------------------------------------------------------
# `_handle_unusable_existing_store` — onboard's own call site, formats `reclaim_or_refuse`'s
# two outcomes and never lets `UnusableStore` (or a raw RuntimeError) reach a user unhandled.
# ---------------------------------------------------------------------------


def test_handle_unusable_existing_store_reclaims_and_returns_normally(tmp_path, capsys):
    (tmp_path / ".beads").mkdir()  # no dolt_mode: safe to reclaim

    onboard._handle_unusable_existing_store(_ctx(tmp_path, furnish=True))  # must not raise

    assert not (tmp_path / ".beads").exists()
    err = capsys.readouterr().err
    assert "⚠" in err
    assert "cleared as wreckage" in err


def test_handle_unusable_existing_store_refuses_legibly_never_raw(tmp_path, capsys):
    import json
    ...
    with pytest.raises(typer.Exit) as exc:
        onboard._handle_unusable_existing_store(_ctx(tmp_path, furnish=True))

    assert exc.value.exit_code == 1
    err = capsys.readouterr().err
    assert "✗" in err
    assert str(tmp_path / ".beads") in err
    # Never the raw exception shape round 4's own bug report showed reaching a user.
    assert "RuntimeError" not in err
    assert "UnusableStore" not in err
    assert "Traceback" not in err
    assert (tmp_path / ".beads").exists()  # refused, not deleted


# ---------------------------------------------------------------------------
# `_act_bd_init`'s third branch end to end: present-but-not-working never skips as ready.
# ---------------------------------------------------------------------------


def test_act_bd_init_reclaims_wreckage_and_mints_a_fresh_store_same_call(tmp_path, monkeypatch):
    """No persisted dolt_mode: reclaimed, then falls through to a REAL mint in the SAME call
    — never a false "already initialized", never a second manual re-run needed."""
    _patch_neighbors(monkeypatch)
    (tmp_path / ".beads").mkdir()  # wreckage: no metadata.json
    monkeypatch.setattr(store_locator, "ensure_server_mode_persisted", lambda base: False)
    calls: list[list[str]] = []

    def _fake_run(cmd, **kw):  # noqa: ARG001
        calls.append(list(cmd))
        # `store_opens`'s own probe against this wreckage must fail — everything else
        # (the real mint that follows reclaim) succeeds trivially.
        if "status" in cmd:
    ...
    assert any(c[:2] == ["bd", "init"] for c in calls)  # a real mint ran, not a false-ready


def test_act_bd_init_refuses_on_a_persisted_but_non_opening_store_never_reports_ready(
    tmp_path, monkeypatch, capsys
):
    """The core acceptance bar (bh-areg.7's review, round 4): a `.beads/` carrying a
    persisted `dolt_mode` that does not open — the SIGKILL-at-4.5s shape — must NEVER be
    reported ready: no `ℹ already initialized`, no exit 0, and no unhandled exception."""
    import json

    _patch_neighbors(monkeypatch)
    (tmp_path / ".beads").mkdir()
    (tmp_path / ".beads" / "metadata.json").write_text(json.dumps({"dolt_mode": "server"}))
    monkeypatch.setattr(hive, "run", lambda cmd, **kw: _Result(returncode=1))  # store_opens fails
    ...
    assert "already initialized" not in out.out  # never the false-ready message
    assert "✗" in out.err
