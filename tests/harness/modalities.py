...
def branch_for(rig: Rig, bead: str) -> tuple[Path, str]:
    _entry, _main, target, branch = worktree.locate(config.load(), rig.prefix, bead)
...
class RemoteSandboxModality(Modality):
    ...
    def assign(self, rig: Rig, bead: str):
        ...
        beads.bd("update", bead, "--assignee", self._dev.name, cwd=rig.main, capture=True)
        beads.push(rig.main)
    ...
    def develop(self, rig: Rig, bead: str):
        ...
        _target, branch = branch_for(rig, bead)
...
def run_flow(rig: Rig, bead_ids: list[str], modality: Modality, label: str = "") -> list[str]:
    ...
    progress(f"▶ {label or rig.prefix} ({modality.name}): {len(bead_ids)} beads")
    remaining = list(bead_ids)
    order: list[str] = []
    while remaining:
        ready = [b for b in beads.ready_ids(rig.main) if b in remaining]
        assert ready, f"deadlock — none ready of {remaining}"
        for bead in ready:
            progress(f"  · {bead}: assign")
            modality.assign(rig, bead)
            progress(f"  · {bead}: develop")
            modality.develop(rig, bead)
            beads.resolve_gates(rig.main, bead)  # review approves
            progress(f"  · {bead}: merge")
            _merge(rig, bead)
            beads.close(rig.main, bead, actor=rig.world.refiner.name)
