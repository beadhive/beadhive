...
def _stop_server_best_effort(path):
    ...
    bh-u562.1 finding 1) — it does not die with this test. Stop it explicitly so the suite
    doesn't accumulate orphaned `dolt sql-server` processes across runs."""
    from beadhive.run import run

    run(["bd", "-C", str(path), "dolt", "stop"], check=False, capture=True, timeout=30)


def test_server_mode_hq_backup_and_restore_real_round_trip(tmp_path, monkeypatch):
    src = tmp_path / "hq-src"
    dst = tmp_path / "hq-dst"
    try:
        ...
        # Now DESTROY the source entirely — the scenario a restore recovers from. Stop its
        # server first (own cleanup) so nothing is left pointing at a deleted data directory.
        _stop_server_best_effort(src)
        import shutil
