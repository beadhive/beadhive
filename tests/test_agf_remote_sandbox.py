...
def test_remote_sandbox_state_transfer_and_signing(world):
    ...
    hive = make_hive(world, work=mod.work_block(), with_remotes=True)
    ids = graph.independent(hive, 2)

    order = run_flow(hive, ids, mod, label="remote-sandbox")
    ...
    remote_branches = set(history.remote_branches(hive.git_remote))
    for bead in ids:
    ...
    actual = Timeline.from_actual("remote-sandbox", hive)
