...
def test_merge_grants_both_arrays():
    out = rig._merge_sandbox_grant({}, "~/wts/github/o/r", SUFFIX)
...
def test_merge_is_self_healing_on_relocation():
    ...
    out = rig._merge_sandbox_grant(existing, "~/new/github/o/r", SUFFIX)
...
def test_merge_is_idempotent():
    once = rig._merge_sandbox_grant({}, "~/wts/github/o/r", SUFFIX)
    twice = rig._merge_sandbox_grant(once, "~/wts/github/o/r", SUFFIX)
    assert twice["sandbox"]["filesystem"]["allowWrite"] == ["~/wts/github/o/r"]


def test_merge_preserves_unrelated_and_other_rigs():
    existing = {
        "permissions": {
    ...
    out = rig._merge_sandbox_grant(existing, "~/wts/github/o/r", SUFFIX)
...
def test_merge_does_not_mutate_input():
    ...
    rig._merge_sandbox_grant(existing, "~/new/github/o/r", SUFFIX)
...
def test_subtree_is_home_relative_and_matches_wt_dir(monkeypatch):
    ...
    sub = rig._sandbox_subtree({}, "github", "o", "r")
...
def test_grant_is_current_detects_stale(tmp_path, monkeypatch):
    ...
    assert rig.grant_is_current({}, clone, "github", "o", "r") is False  # root moved → stale
    # no grant present at all → None (rig opted out, not a drift warning)
    assert rig.grant_is_current({}, tmp_path / "bare", "github", "o", "r") is None


def test_install_grant_is_noop_when_ephemeral(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    rig._install_sandbox_grant({"worktrees": {"ephemeral": True}}, "github", "o", "r")
...
def test_install_grant_writes_when_persistent(tmp_path, monkeypatch):
    ...
    rig._install_sandbox_grant({"worktrees": {"ephemeral": False}}, "github", "o", "r")
